...
class helper_mssql
{
    ...
    public function connect($server = null, $db = null, $user = null, $pass = null)
    {
        if (is_null($server)) {
            $server = \Helpers\Config::MSSQL_DB_SERVER();
        }
        if (is_null($db)) {
            $db = \Helpers\Config::MSSQL_DB_NAME();
        }
        if (is_null($user)) {
            $user = \Helpers\Config::MSSQL_DB_USER();
        }
        if (is_null($pass)) {
            $pass = \Helpers\Config::MSSQL_DB_PASS();
        }
        $this->server = $server;
        $this->user = $user;
        $this->pass = $pass;
    ...
    public function output_table()
    {
        ...
        echo "<table class=".($this->datatables ? "'table table-striped table-bordered table-hover dataTable'" : "'table simpletable'")." ".($this->table_style ? 'style=\''.$this->table_style.'\'' : '')." border='0'>";
        echo "<thead>";
        for ($f = 0; $f <= $this->field_count-1; $f++) {
            ...
            if (isset($this->columns)) {
                //Only show columns if in list of visible columns
                    if (!in_array(strtolower($field->name), array_map('strtolower', $this->columns))) {
                        $show = false;
                    }
            }
            if ($show) {
