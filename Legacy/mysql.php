...
class helper_mysql
{
    ...
    {
        if (is_null($server)) {
            $server = \Helpers\Config::MYSQL_DB_SERVER();
        }
        if (is_null($db)) {
            $db = \Helpers\Config::MYSQL_DB_NAME();
        }
        if (is_null($user)) {
            $user = \Helpers\Config::MYSQL_DB_USER();
        }
        if (is_null($pass)) {
            $pass = \Helpers\Config::MYSQL_DB_PASS();
        }
        if (is_null($port)) {
            $port = \Helpers\Config::MYSQL_DB_PORT();
        }
        $this->server = $server;
        $this->port = $port;
        $this->user = $user;
    ...
    public function isnull($value, $default)
    {
        ...
        if (!is_numeric($ret) && strtolower($ret) != 'null') {
            $ret = "'".$ret."'";
        }
        return $ret;
    }
