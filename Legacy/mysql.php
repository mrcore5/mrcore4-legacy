...
class helper_mysql {
	...
	public function connect($server = null, $db = null, $user = null, $pass = null, $port = null) {
		if (is_null($server)) $server = \Helpers\Config::MYSQL_DB_SERVER();
		if (is_null($db)) $db = \Helpers\Config::MYSQL_DB_NAME();
		if (is_null($user)) $user = \Helpers\Config::MYSQL_DB_USER();
		if (is_null($pass)) $pass = \Helpers\Config::MYSQL_DB_PASS();
		if (is_null($port)) $port = \Helpers\Config::MYSQL_DB_PORT();
		$this->server = $server;
		$this->port = $port;
		$this->user = $user;
		$this->pass = $pass;
