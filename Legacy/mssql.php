...
class helper_mssql {
	...
	public function connect($server = null, $db = null, $user = null, $pass = null) {
		if (is_null($server)) $server = \Helpers\Config::MSSQL_DB_SERVER();
		if (is_null($db)) $db = \Helpers\Config::MSSQL_DB_NAME();
		if (is_null($user)) $user = \Helpers\Config::MSSQL_DB_USER();
		if (is_null($pass)) $pass = \Helpers\Config::MSSQL_DB_PASS();
		$this->server = $server;
		$this->user = $user;
		$this->pass = $pass;
		$this->db = $db;
